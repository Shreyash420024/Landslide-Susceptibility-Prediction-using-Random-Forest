...
rainfall_june_july = ds['RAINFALL'].sel(TIME=slice('2022-06', '2022-07')).values

# Create a meshgrid for plotting
lon, lat = np.meshgrid(longitude, latitude)

# Plotting Rainfall and Elevation
...
pcm_rainfall = ax.pcolormesh(lon, lat, rainfall_june_july[0, :, :], shading='auto', cmap='Blues')

# Add colorbar for rainfall
cbar_rainfall = fig.colorbar(pcm_rainfall, cax=cax)
cbar_rainfall.set_label('Rainfall (cm)')
...
pcm_elevation1 = ax.imshow(elevation_data1[0, :, :], cmap='terrain', extent=[transform1[2], transform1[2] + transform1[0] * elevation_data1.shape[2],
                                                                   transform1[5] + transform1[4] * elevation_data1.shape[1], transform1[5]])

# Plot the elevation data 2
pcm_elevation2 = ax.imshow(elevation_data2[0, :, :], cmap='terrain', extent=[transform2[2], transform2[2] + transform2[0] * elevation_data2.shape[2],
                                                                   transform2[5] + transform2[4] * elevation_data2.shape[1], transform2[5]])

# Add colorbar for elevation
cbar_elevation = fig.colorbar(pcm_elevation2, cax=cax)
cbar_elevation.set_label('Elevation (meters)')
...
plt.savefig('combined_plot1.png', bbox_inches='tight')
# or
